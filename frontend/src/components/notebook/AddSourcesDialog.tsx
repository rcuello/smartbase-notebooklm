...
const AddSourcesDialog = ({ open, onOpenChange, notebookId }: AddSourcesDialogProps) => {
        ...
        if (failed > 0) {
          toast({
            ...
            description: `${failed} file${
              failed > 1 ? 's' : ''
            } had processing issues. Check the sources list for details.`,
            variant: 'destructive',
          });
        }
      });
  ...
  const handleMultipleWebsiteSubmit = async (urls: string[]) => {
    ...
    try {
      ...
      if (urls.length > 1) {
        ...
        // Create remaining sources
        remainingSources = await Promise.all(
          urls.slice(1).map(async (url, index) => {
      ...
      toast({
        title: 'Success',
        description: `${urls.length} website${
          urls.length > 1 ? 's' : ''
        } added and sent for processing`,
      });
          ...
          <DialogHeader className='pb-4'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center space-x-2'>
                ...
                <DialogTitle className='text-xl font-medium'>SmartBaseLM</DialogTitle>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className='space-y-6'>
            <div>
              ...
              <p className='text-gray-600 text-sm mb-1'>
                Sources let SmartBaseLM base its responses on the information that matters most to
                you.
              </p>
